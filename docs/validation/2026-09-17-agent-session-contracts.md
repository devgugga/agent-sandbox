...
- `CodexDriver` e `ClaudeDriver`: `resume_supported=True` é alcançável (opção confirmada no `--help` do binário de HOST **e** fonte de session-id comprovada). `probe()` ainda pode reportar `False` em runtime se o `--help` real não confirmar a opção (ex.: versão diferente na imagem).
